...
namespace WinWam6
{
    public class CBusiness : INotifyPropertyChanged
    {
        private TableWrapper lObj;
        private CBusCustom m_custom;
        ...
        public CBusiness(string ID)
        {
            lObj = WWD.GetTableWrapper("Business");
            lObj.Fields["Bus_ID"].Value = ID;
            lObj.Load();

            PhysicalAddress = new CAddress(
                lObj["Addr1"].ToString(),
                lObj["Addr2"].ToString(),
                lObj["City"].ToString(),
                lObj["State"].ToString(),
        ...
        public CAddress PhysicalAddress { get; set; }
        public CAddress MailingAddress { get; set; }
